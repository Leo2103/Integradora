...
public class RecoverPasswordController {
	...
	public String enviarContrasenaEmail(@RequestParam("username") String username,
		...
		// Update password
		boolean respuestaCambio = userServiceImpl.cambiarContrasena(contrasenaEncriptada, user.getNombre());
		// Get full user_name
		String nombreUsuario = user.getNombre().concat(" ").concat(user.getApellidos());
		// Create email content
		String htmlContent = plantillaRecuperacionContrasena(nombreUsuario, user.getNombre(), nuevaContrasena);
		// Send message
		boolean respuestaEmail = emailServiceImpl.sendEmail(user.getNombre(), "Cambio de contraseña", htmlContent);
